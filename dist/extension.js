...
function activate(context) {
    ...
    const catCodingExample = vscode.commands.registerCommand('catCoding.start', () => {
        const panel = vscode.window.createWebviewPanel('catCoding', 'Cat Coding', vscode.ViewColumn.One, { enableScripts: true,
            localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, 'server'))]
        });
        const webviewJsPath = vscode.Uri.file(path.join(context.extensionPath, 'server', 'index.js'));
    ...
    const visualizer = vscode.commands.registerCommand('nextFlow.start', () => {
        const panel = vscode.window.createWebviewPanel('nextFlow', 'Next Flow', vscode.ViewColumn.One, {});
        // panel.webview.html = getWebviewContent();
    });
    context.subscriptions.push(catCodingExample);
}
function getWebviewContent(webviewJsUri) {
    return `<!DOCTYPE html>
	<html lang="en">
	<head>
		<meta charset="UTF-8">
