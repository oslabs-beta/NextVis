  ...
  function update(event, source) {
    ...
    const nodeEnter = node
      ...
      .on("mouseover", function (event, d) {
        ...
        // fetch aggregated performance from your global data
        const perfAggregate = window.middlewarePerformanceMap?.[d.data.name];
        if (perfAggregate) {
          const info3 = g
            .append("text")
            .classed("info", true)
            ...
            .attr("stroke", "white")
            .text(
              `Avg CPU: ${perfAggregate.avgCpu} - Mem: ${perfAggregate.avgMem} - Time: ${perfAggregate.avgMs}ms`
            );
        }
      })
...
window.addEventListener("message", (event) => {
  ...
  switch (message.command) {
    case "filePicked":
      ...
      break;
    case "runtimeMetrics":
      const invocations = message.data;
      // Here you can unify your static parse tree with actual invocation data
      // E.g. highlight "middlewareA" if it has 50 calls vs. 10 for "middlewareB"
      console.log("Runtime Invocations:", invocations);
      break;
    case "newInvocation":
      // append a row for event.data
      break;
  }
});
